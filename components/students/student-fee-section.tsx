...
export function StudentFeeSection({ account }: { account: FeeAccountView | null }) {
  if (!account) {
    return (
      <section className="space-y-3">
        ...
        <div className="bg-white rounded-xl border border-gurukul-gray shadow-subtle px-5 py-8 text-center">
          ...
          <p className="text-sm text-slate-500">No fee account set up for this student yet.</p>
          <p className="text-[11px] text-slate-400 mt-1">An administrator can set fees from the Student Registry.</p>
        </div>
      </section>
    );
  }
  ...
  const paidPercent = account.amountDue > 0 ? Math.min(100, Math.round((account.amountPaid / account.amountDue) * 100)) : 0;
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-semibold ${statusCls}`}>
          {account.status}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-gurukul-gray shadow-subtle p-4 text-center">
          <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Total Fee</p>
          <p className="text-lg font-bold text-gurukul-dark mt-1">{formatCurrency(account.amountDue)}</p>
          <p className="text-[10px] text-slate-400 mt-0.5">{account.academicYear}</p>
        </div>
        <div className="bg-white rounded-xl border border-gurukul-gray shadow-subtle p-4 text-center">
          <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Paid</p>
          <p className="text-lg font-bold text-emerald-600 mt-1">{formatCurrency(account.amountPaid)}</p>
          <p className="text-[10px] text-slate-400 mt-0.5">{paidPercent}% collected</p>
        </div>
        <div className="bg-white rounded-xl border border-gurukul-gray shadow-subtle p-4 text-center">
          <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Remaining</p>
          <p className={`text-lg font-bold mt-1 ${remaining > 0 ? "text-gurukul-dark" : "text-emerald-600"}`}>
      ...
      <div className="bg-white rounded-xl border border-gurukul-gray shadow-subtle p-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 pt-4 border-t border-slate-100 text-xs">
          <div>
            <p className="text-[10px] text-slate-400 font-medium uppercase">Academic year</p>
            <p className="font-semibold text-gurukul-dark mt-0.5">{account.academicYear}</p>
          </div>
          <div>
            <p className="text-[10px] text-slate-400 font-medium uppercase">Due date</p>
            <p className="font-semibold text-gurukul-dark mt-0.5">{formatDate(account.dueDate)}</p>
          </div>
          <div>
            <p className="text-[10px] text-slate-400 font-medium uppercase">Payments made</p>
            <p className="font-semibold text-gurukul-dark mt-0.5">{account.payments.length}</p>
          </div>
          <div>
            <p className="text-[10px] text-slate-400 font-medium uppercase">Status</p>
            <p className="font-semibold text-gurukul-dark mt-0.5">{account.status}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gurukul-gray shadow-subtle overflow-hidden">
        <div className="px-5 py-3 border-b border-gurukul-gray flex items-center gap-2">
          ...
          <h3 className="text-xs font-bold text-gurukul-dark">Payment History</h3>
          {account.payments.length > 0 && (
            <span className="text-[10px] text-slate-400 ml-auto">{account.payments.length} transaction{account.payments.length === 1 ? "" : "s"}</span>
          )}
        </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {account.payments.map((p) => (
                  <tr key={p.id} className="hover:bg-slate-50/60 transition-colors">
                    <td className="px-5 py-3 text-slate-600">{formatDate(p.paidAt)}</td>
                    <td className="px-5 py-3 font-bold text-gurukul-dark">{formatCurrency(p.amount)}</td>
                    ...
                    <td className="px-5 py-3 font-mono text-[10px] text-slate-400">{p.receiptNo}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="bg-slate-50 border-t border-gurukul-gray">
                  <td className="px-5 py-3 font-semibold text-slate-600">Total paid</td>
                  <td className="px-5 py-3 font-bold text-emerald-700" colSpan={3}>
