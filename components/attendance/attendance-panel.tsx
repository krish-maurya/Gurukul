...
import { Calendar, CheckCircle2, ClipboardCheck, Edit3, Filter, GraduationCap, Send } from "lucide-react";
...
export function AttendancePanel({ focusMode = false }: AttendancePanelProps) {
  ...
  return (
    <div className={`${focusMode ? "min-h-screen bg-gurukul-white" : ""} space-y-4 pb-24 sm:space-y-5 sm:pb-28 animate-fade-in`}>
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <h1 className="text-sm font-semibold tracking-tight text-gurukul-dark truncate">Take Attendance</h1>
                </div>
                <p className="text-[10px] text-neutral-400 truncate">{currentUser?.name || "Faculty"} · {date}</p>
              </div>
            </div>
      ...
      <div className={focusMode ? "mx-auto max-w-5xl px-4 pt-4 sm:px-6" : ""}>
        ...
        {!focusMode && (
          <div className="flex flex-col gap-3 border-b border-neutral-200 pb-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            {isSubmitted && (
              <button onClick={() => setIsSubmitted(false)} className="btn-secondary btn-sm">
                <Edit3 className="h-3 w-3" />
                <span>Edit</span>
              </button>
            )}
        ...
        <div className="card flex flex-col gap-3 p-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
          ...
          {focusMode && (
            <p className="text-[11px] text-neutral-500">Tap a roll to mark absent · Submit when done</p>
          )}
        </div>
          ...
          <div className="flex items-center justify-between gap-3 card p-3 text-xs text-neutral-600">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span>
                ...
                {focusMode ? " — tap Edit to make changes, or go to Dashboard when done." : " — tap Edit (top right) to make changes."}
              </span>
            </div>
            {focusMode && (
              <button
            ...
            <div className="grid max-w-6xl grid-cols-3 gap-2 min-[420px]:grid-cols-4 sm:grid-cols-5 sm:gap-3 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10">
              {students.map((student) => {
                ...
                return (
                  ...
                  >
                    <span className={`text-[9px] font-medium ${isAbsent ? "text-red-100" : "text-neutral-400"}`}>ROLL</span>
                    <span className={`font-mono text-lg font-bold leading-tight ${isAbsent ? "text-white" : "text-gurukul-dark"}`}>#{student.rollNumber}</span>
                    <span className={`mt-0.5 text-[8px] font-medium tracking-wider ${isAbsent ? "text-red-100" : "text-neutral-400"}`}>{student.status}</span>
                  </button>
                );
              })}
            </div>
      ...
      {/* Bottom Bar */}
      <div className={`${bottomBarClass} flex items-center justify-between gap-3 border-t border-neutral-200 bg-white px-3 py-3 shadow-subtle sm:gap-4 sm:p-4 sm:px-8`}>
        <div className="flex min-w-0 items-center gap-4 sm:gap-6">
          <div>
            <p className="text-[9px] font-medium uppercase tracking-wider text-neutral-400">Summary</p>
            <p className="mt-0.5 truncate text-xs font-semibold text-gurukul-dark">{grade} · {date}</p>
          </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            onClick={() => (focusMode && isSubmitted ? router.push("/") : setIsModalOpen(true))}
            disabled={(!focusMode && isSubmitted) || isLoading || students.length === 0}
            className="btn-primary btn-sm shrink-0"
          >
            <Send className="h-3.5 w-3.5" />
            <span>{isSubmitted ? (focusMode ? "Go to Dashboard" : "Submitted") : "Review & Submit"}</span>
          </button>
        </div>
      </div>
      ...
      {result && (
        <SubmissionResultModal
          ...
          doneLabel={focusMode && result.type === "success" ? "Go to Dashboard" : undefined}
        />
      )}
    </div>
  );
