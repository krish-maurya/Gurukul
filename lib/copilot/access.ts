...
export async function getTeacherGrades(staffId: string | null | undefined): Promise<string[]> {
  if (!staffId) return [];
...
export async function getDefaultTeacherGrade(staffId: string | null | undefined): Promise<string | undefined> {
...
export function teacherCanAccessGrade(context: CopilotContext, grade: string | undefined, allowedGrades: string[]): boolean {
  if (context.role === "ADMIN" || !grade) return true;
  return allowedGrades.includes(grade);
}

export function denyGradeAccess(context: CopilotContext, grade: string, allowedGrades: string[]): string {
  return `You don't have access to ${grade}. Your assigned classes: ${allowedGrades.length ? allowedGrades.join(", ") : "none on file"}.`;
}

export type CopilotHistoryItem = { role: string; content: string; intent?: string };

/** Merge short follow-ups with prior user question for better DB retrieval. */
export function enrichQueryWithHistory(query: string, history: CopilotHistoryItem[]): string {
  const q = query.trim();
  ...
  const isFollowUp =
    isShort &&
    (/\b(again|same|repeat|that|this|those|them|it|him|her|yes|ok|sure)\b/i.test(q) ||
      /\b(what about|how about|and for|now for)\b/i.test(q) ||
      (lastAssistant?.intent && !/\b(who|which|how many|show|list|open)\b/i.test(q)));
  ...
  if (/\b(what about|how about)\b/i.test(q)) {
    const modifier = q.replace(/\b(what|how|about|and|for|the|now)\b/gi, " ").trim();
    if (modifier) return `${lastUser.content} ${modifier}`;
  }

  // Carry forward intent context: "roll 7" after "who is absent in 10A"
  if (lastAssistant?.intent === "ATTENDANCE_QUERY" && /\broll\b/i.test(q) && !/absent|present|attendance/i.test(q)) {
