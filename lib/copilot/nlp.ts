...
type NlpRequest = {
  intent?: "attendance_status" | "attendance" | "fees" | "student" | "timetable" | "staff" | "stats" | "document" | "navigation" | "action" | "help" | "out_of_scope";
  studentName?: string;
  teacherName?: string;
  grade?: string;
  day?: string;
...
export function isCasualGreeting(query: string) {
  return /^(?:h+[iey]+|hello+|hey+|good\s+(?:morning|afternoon|evening)|thanks?|thank\s+you|how\s+are\s+you)[!,.?\s]*$/i.test(query.trim());
}
...
export async function generateCasualReply(query: string, name: string): Promise<string | null> {
  if (!process.env.GROQ_API_KEY || !isCasualGreeting(query)) return null;
  try {
    const completion = await groqClient().chat.completions.create({
      model: process.env.GROQ_MODEL || "openai/gpt-oss-20b",
      messages: [
        { role: "system", content: "You are the friendly GURUKUL school-management assistant. Reply warmly to this casual greeting in one short sentence, greet the user by first name when natural, then offer help with school records, attendance, timetable, documents, or fees. Do not answer general-knowledge questions or claim access to data." },
...
function canonicalRequest(original: string, request: NlpRequest): string {
  ...
  const date = /^\d{4}-\d{2}-\d{2}$/.test(request.date || "") ? request.date : "";
  ...
  switch (request.intent) {
    case "attendance_status": return `attendance status is ${name || "student"} present ${grade} ${date}`;
    case "attendance": return `attendance ${name || ""} ${grade} ${date}`;
    case "fees": return `pending fees ${name || ""} ${grade}`;
    case "student": return `student profile ${name || ""} ${grade}`;
    case "timetable": return `timetable ${teacher || ""} ${grade || ""} ${day || ""} ${subject || ""}`.replace(/\s+/g, " ").trim();
    case "staff": return `staff ${teacher || name || ""} ${subject || ""}`.replace(/\s+/g, " ").trim();
    case "stats": return `how many students ${grade}`;
    case "document": return `school policy document ${original}`;
    case "navigation": return `open ${request.destination || original}`;
    case "action": return `mark attendance ${name || ""} ${grade} ${date}`;
...
export async function understandCopilotRequest(
  ...
  try {
    ...
    const text = completion.output_text.trim().replace(/^```json\s*|\s*```$/g, "");
...
function resolveFollowUpLocally(
  ...
  const isFollowUp =
    words.length <= 5 &&
    (/\b(again|repeat|same|more|also|too|show|see|check|view|refresh|update|another|else|other)\b/i.test(q) ||
  ...
  if (/\b(what about|how about|and|and in|now)\b/i.test(q)) {
    const modifier = q.replace(/\b(what|how|and|now|about|in|the|for)\b/gi, "").trim();
...
export async function polishGroundedResponse(
  ...
  if (!process.env.GROQ_API_KEY || response.intent === "OUT_OF_SCOPE" || !response.sources?.length || response.data?.rows.length) return FALLBACK(response);
  try {
    const client = groqClient();
    const evidence = JSON.stringify({ message: response.message, data: response.data, sources: response.sources });
    const completion = await client.responses.create({
      model: process.env.GROQ_MODEL || "openai/gpt-oss-20b",
      instructions: "You are GURUKUL's concise school-management assistant. Rewrite the supplied verified answer in calm, clear language. Use ONLY the supplied evidence. Never add, infer, alter, or omit factual values, names, dates, URLs, or permissions. If evidence is absent, say it could not be verified. Do not answer requests outside school operations. Return plain prose only: no Markdown, tables, headings, lists, citations, or labels.",
