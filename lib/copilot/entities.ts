...
const TIMETABLE_STOP_WORDS = new Set([
  ...
  "a", "an", "the", "for", "of", "in", "on", "at", "to", "and", "or", "is", "are", "was", "were",
  "has", "have", "had", "do", "does", "did", "be", "been", "being", "not", "no", "yes", "its",
  // pronouns & common verbs
  "show", "give", "get", "tell", "me", "my", "mine", "our", "please", "need", "want", "what", "which",
  "who", "when", "where", "how", "can", "could", "would", "should", "will", "shall",
  "i", "you", "he", "she", "it", "we", "they", "them", "his", "her", "your", "their",
  // conversational words (critical for name extraction)
  "about", "named", "called", "name", "details", "detail", "info", "information",
  "data", "record", "records", "check", "know", "find", "lookup", "look",
  "this", "that", "there", "here", "with", "from", "by", "also", "just", "only",
  "any", "some", "if", "whether", "then", "so", "but", "like", "such",
  // timetable-specific
  "timetable", "schedule", "class", "classes", "grade", "section", "period",
  "periods", "room", "subject", "teacher", "teachers", "prof", "professor", "dr", "sir", "mr", "mrs",
  "ms", "whole", "full", "complete", "all", "school", "master", "weekly", "day", "today", "tomorrow",
]);
...
export function requestedGrade(query: string): string | undefined {
  ...
  if (compact && /class|grade|timetable|schedule|student|attendance|section/i.test(query)) {
...
export function requestedDay(query: string): string | undefined {
  const match = query.toLowerCase().match(/\b(monday|tuesday|wednesday|thursday|friday|mon|tue|tues|wed|thu|thur|thurs|fri)\b/);
  return match ? DAY_ALIASES[match[1]] : undefined;
}

export function isMyScheduleQuery(query: string) {
  return /\b(my|mine|me)\b/i.test(query) && /\b(timetable|schedule|periods?|classes?\b)/i.test(query);
}

export function isConflictQuery(query: string) {
  return /\b(conflicts?|clashes?|collisions?|double[- ]book(?:ed|ing)?|overlaps?)\b/i.test(query);
}

export function nameTerms(query: string, extraStopWords: string[] = []): string[] {
  const stopWords = new Set([...TIMETABLE_STOP_WORDS, ...extraStopWords.map((word) => word.toLowerCase())]);
  return query
    .replace(/[^a-z0-9\s.'-]/gi, " ")
    .split(/\s+/)
    .map((term) => term.replace(/^['"]|['"]$/g, "").trim())
    .filter((term) => term.length > 1 && !stopWords.has(term.toLowerCase()) && !/^\d+$/.test(term))
    .slice(0, 4);
}

export function subjectTerms(query: string): string[] {
  const match = query.match(/\b(?:subject|teaches?|teaching)\s+([a-z][a-z\s&]{2,40})/i);
...
export async function resolveStaffFromContext(context: CopilotContext) {
  ...
  for (let size = Math.min(3, parts.length); size >= 1; size--) {
    ...
    if (matches.length > 1) {
      const exact = matches.find((staff) => staff.name.toLowerCase() === context.name.toLowerCase());
...
export async function resolveStaffFromQuery(query: string, context: CopilotContext) {
  ...
  if (matches.length === 1) return matches[0];
  if (matches.length > 1) {
    const best = matches.find((staff) => terms.every((term) => staff.name.toLowerCase().includes(term.toLowerCase())));
    return best || null;
  }
  return null;
}
...
export async function knownGrades() {
  const [timetableGrades, studentGrades] = await Promise.all([
    prisma.timetableSlot.findMany({ select: { grade: true }, distinct: ["grade"] }),
    prisma.student.findMany({ select: { grade: true }, distinct: ["grade"] }),
  ]);
  return [...new Set([...timetableGrades, ...studentGrades].map((item) => item.grade))].sort();
}
...
export function formatFilterDescription(filters: { grade?: string; teacher?: string; day?: string; room?: string; period?: number }) {
  const parts = [
...
export function requestedRollNumber(query: string): number | undefined {
  ...
  if (shortMatch && /student|class|grade|who|find|show|tell|\d{1,2}[a-z]\b/i.test(query)) return Number(shortMatch[1]);
  return undefined;
}
...
export function requestedRoom(query: string): string | undefined {
  ...
  if (lab) return lab[1].split(/\s+/).map((w) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(" ");
  return undefined;
}
...
export function requestedPeriod(query: string): number | undefined {
  ...
  const explicit = query.match(/\bperiod\s*(\d)\b/i) || query.match(/\bP(\d)\b/);
  if (explicit) { const n = Number(explicit[1]); if (n >= 1 && n <= 8) return n; }
