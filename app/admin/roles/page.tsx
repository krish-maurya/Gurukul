    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="border-b pb-5" style={{ borderColor: "var(--line)" }}>
        <div className="flex items-center gap-2">
          <h1 className="text-xl font-bold text-gurukul-ink tracking-tight" style={{ fontFamily: "var(--font-syne)" }}>Audit & Access Control</h1>
      ...
      {!isAdmin && (
        <div className="flex items-start gap-3 p-4 rounded-xl text-xs" style={{ background: "var(--soft)", border: "1px solid var(--line)" }}>
          <ShieldAlert className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: "var(--muted)" }} />
          <div>
            <p className="font-semibold text-gurukul-ink">Restricted View (Staff Role Active)</p>
      ...
      <div className="card overflow-hidden">
        <div className="px-5 py-4 border-b" style={{ borderColor: "var(--line)", background: "var(--soft)" }}>
          <h3 className="text-sm font-semibold text-gurukul-ink" style={{ fontFamily: "var(--font-syne)" }}>Access Permission Boundaries</h3>
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr style={{ background: "var(--soft)", borderColor: "var(--line)" }} className="border-b">
                <th className="px-5 py-3 text-[10px] font-semibold uppercase tracking-wider" style={{ color: "var(--faint)" }}>Permission / Capability</th>
                <th className="px-5 py-3 text-[10px] font-semibold uppercase tracking-wider" style={{ color: "var(--faint)" }}>Scope Description</th>
                <th className="px-5 py-3 text-[10px] font-semibold uppercase tracking-wider text-center" style={{ color: "var(--faint)" }}>Administrator</th>
                <th className="px-5 py-3 text-[10px] font-semibold uppercase tracking-wider text-center" style={{ color: "var(--faint)" }}>Staff / Faculty</th>
              </tr>
            </thead>
            <tbody className="divide-y" style={{ borderColor: "var(--hover)" }}>
              {PERMISSION_MATRIX.map((row) => (
                <tr key={row.permission} className="transition-colors" style={{ background: "transparent" }}
                  ...
                  <td className="px-5 py-4 max-w-sm" style={{ color: "var(--muted)" }}>{row.description}</td>
