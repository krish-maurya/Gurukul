...
export default function ParentConnectPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-5" style={{ borderColor: "var(--line)" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gurukul-ink" style={{ fontFamily: "var(--font-syne)" }}>Parent Connect</h1>
            <p className="text-xs" style={{ color: "var(--muted)" }}>Every message is reviewed and sent by a teacher — parents read them on their private portal.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={handleGenerate} className="btn-secondary font-medium text-xs px-3.5 py-2.5 flex items-center gap-1.5">
            <Sparkles className="w-4 h-4" style={{ color: "var(--accent)" }} />
            <span>Draft Fee Reminders</span>
          </button>
          <button onClick={() => setShowCompose(true)} className="btn-primary font-medium text-xs px-4 py-2.5 flex items-center gap-2">
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:items-center justify-between">
        <div className="flex items-center gap-1 p-1 rounded-xl w-fit" style={{ background: "var(--hover)", border: "1px solid var(--line)" }}>
          {([
            ...
            <button key={key} onClick={() => setTab(key)}
              ...
              style={tab === key ? { background: "#ffffff", color: "var(--ink)", boxShadow: "0 1px 3px rgba(15, 23, 42, 0.06)" } : { color: "var(--muted)" }}>
              {label}
            </button>
          ))}
        </div>
      ...
      {/* Message list */}
      <div className="bg-white rounded-xl shadow-subtle divide-y overflow-hidden" style={{ border: "1px solid var(--line)", borderColor: "var(--line)" }}>
        {isLoading ? (
          <p className="text-sm text-center py-12" style={{ color: "var(--faint)" }}>Loading messages...</p>
        ) : messages.length === 0 ? (
          <div className="text-center py-12">
            <MessagesSquare className="w-8 h-8 mx-auto mb-2" style={{ color: "var(--line-strong)" }} />
            <p className="text-sm" style={{ color: "var(--faint)" }}>{tab === "DRAFT" ? "No drafts waiting. Compose a message or submit attendance to auto-draft absence notes." : "Nothing here yet."}</p>
          </div>
        ) : (
          messages.map((m) => {
            ...
            return (
              <div key={m.id} className="px-5 py-4 transition-colors">
                <div className="flex items-start gap-3">
                  <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0 mt-0.5" style={{ background: "var(--accent-soft)", color: "var(--accent-text)" }}>
                    <Icon className="w-4 h-4" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-xs font-bold text-gurukul-ink">{m.student.name}</span>
                      <span className="text-[10px]" style={{ color: "var(--faint)" }}>{m.student.grade} · Roll {m.student.rollNumber}</span>
                    ...
                    <p className="text-xs font-semibold mt-1" style={{ color: "var(--ink)" }}>{m.title}</p>
                    <p className="text-[11px] mt-0.5 line-clamp-2 whitespace-pre-line" style={{ color: "var(--muted)" }}>{m.body}</p>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button onClick={() => setLinkModal(m.student)} title="Parent portal link"
                      className="p-2 rounded-lg transition-colors" style={{ color: "var(--faint)" }}
                    ...
                    {m.status === "DRAFT" && (
                      <>
                        <button onClick={() => handleDelete(m.id)} disabled={busyId === m.id} title="Discard draft"
                          className="p-2 rounded-lg transition-colors disabled:opacity-50" style={{ color: "var(--faint)" }}
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-base font-bold text-gurukul-ink flex items-center gap-2" style={{ fontFamily: "var(--font-syne)" }}>
            <PenSquare className="w-4 h-4" style={{ color: "var(--accent)" }} /><span>Message to Parents</span>
          </h2>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-1 p-1 rounded-xl" style={{ background: "var(--hover)", border: "1px solid var(--line)" }}>
            <button type="button" onClick={() => setMode("STUDENT")}
              className="text-xs font-semibold py-2 rounded-lg transition-all"
              style={mode === "STUDENT" ? { background: "#ffffff", color: "var(--ink)", boxShadow: "0 1px 3px rgba(15, 23, 42, 0.06)" } : { color: "var(--muted)" }}>
              Specific students
            </button>
            <button type="button" onClick={() => setMode("GRADE")}
              className="text-xs font-semibold py-2 rounded-lg transition-all"
              style={mode === "GRADE" ? { background: "#ffffff", color: "var(--ink)", boxShadow: "0 1px 3px rgba(15, 23, 42, 0.06)" } : { color: "var(--muted)" }}>
              Whole class
            </button>
          </div>
          ...
          {mode === "STUDENT" ? (
            <div>
              <label className="text-xs font-semibold mb-1.5 block" style={{ color: "var(--muted)" }}>Students</label>
              {selectedStudents.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {selectedStudents.map((s) => (
                    <span key={s.id} className="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-1 rounded-lg" style={{ background: "var(--accent-soft)", color: "var(--accent-text)" }}>
                      {s.name} <span style={{ color: "var(--accent-text)", opacity: 0.6 }}>({s.grade})</span>
              ...
              {matches.length > 0 && (
                <div className="mt-1 rounded-lg divide-y overflow-hidden" style={{ border: "1px solid var(--line)", borderColor: "var(--hover)" }}>
                  {matches.map((s) => (
                    <button type="button" key={s.id}
                      ...
                      <span className="font-medium text-gurukul-ink">{s.name}</span>
                      <span style={{ color: "var(--faint)" }}>{s.grade} · Roll {s.rollNumber}</span>
                    </button>
                  ))}
                </div>
              )}
            ...
            <div>
              <label className="text-xs font-semibold mb-1.5 block" style={{ color: "var(--muted)" }}>Class</label>
          ...
          <div>
            <label className="text-xs font-semibold mb-1.5 block" style={{ color: "var(--muted)" }}>Subject</label>
            <input required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Parent-teacher meeting on Friday"
              className="input w-full text-sm px-3 py-2.5 rounded-lg" />
          </div>
          <div>
            <label className="text-xs font-semibold mb-1.5 block" style={{ color: "var(--muted)" }}>Message</label>
          ...
          <p className="text-[11px] text-center" style={{ color: "var(--faint)" }}>Drafts are never delivered automatically — you press Send when ready.</p>
        </form>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 backdrop-blur-sm z-50 flex items-center justify-center p-4" style={{ background: "rgba(17, 19, 18, 0.5)" }}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-bold text-gurukul-ink flex items-center gap-2" style={{ fontFamily: "var(--font-syne)" }}>
            <Link2 className="w-4 h-4" style={{ color: "var(--accent)" }} /><span>Parent Portal — {student.name}</span>
          </h2>
          <button onClick={onClose} className="p-1.5 rounded-lg transition-colors" style={{ color: "var(--faint)" }}><X className="w-4 h-4" /></button>
        </div>
        ...
        <label className="text-xs font-semibold mb-1.5 block" style={{ color: "var(--muted)" }}>Parent email</label>
        ...
        {status && <p className="text-[11px] font-medium bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 mb-3" style={{ color: "var(--muted)" }}>{status}</p>}

        {url && (
          <div className="flex items-center gap-2">
            <input readOnly value={url} onFocus={(e) => e.target.select()}
              className="input flex-1 text-[11px] font-mono px-3 py-2.5 rounded-lg" />
            <button onClick={copy} className="btn-primary shrink-0 text-xs font-medium px-3 py-2.5 rounded-lg flex items-center gap-1.5">
