...
export async function POST(req: Request) {
  try {
    ...
    if (typeof grade !== "string" || typeof date !== "string" || !Array.isArray(entries) || entries.length === 0) {
      return NextResponse.json({ error: "Choose a class and include at least one student." }, { status: 400 });
    }

    const requestedStudentIds = entries.map((entry: { studentId?: unknown }) => entry.studentId).filter(
      (studentId: unknown): studentId is string => typeof studentId === "string"
    );
    ...
    if (validStudents.length !== entries.length) {
      return NextResponse.json({ error: "The class list changed. Refresh the page and try again." }, { status: 400 });
    }
    const studentsById = new Map(validStudents.map((student) => [student.id, student]));
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const entriesToCreate = entries.map((e: { studentId: string; status: string }) => ({
