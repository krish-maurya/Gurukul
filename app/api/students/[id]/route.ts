...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const required = { name: str(body.name), dob: str(body.dob), grade: str(body.grade), parentName: str(body.parentName), contact: str(body.contact) };
    ...
    const student = await prisma.student.update({
      ...
      data: {
        ...
        ...(required.parentName !== undefined ? { parentName: required.parentName } : {}),
        ...(required.contact !== undefined ? { contact: required.contact } : {}),
        ...(parentEmail !== undefined ? { parentEmail: parentEmail || null } : {}),
        ...(str(body.address) !== undefined ? { address: str(body.address) || null } : {}),
        ...(str(body.medicalNotes) !== undefined ? { medicalNotes: str(body.medicalNotes) || null } : {}),
        ...(str(body.previousSchool) !== undefined ? { previousSchool: str(body.previousSchool) || null } : {}),
        ...(status ? { status } : {}),
      },
    });
