...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { dob, grade, parentName, parentEmail, contact, address, medicalNotes, previousSchool } =
    ...
    if (normalizedParentEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedParentEmail)) {
      return NextResponse.json({ error: "Parent email address is invalid" }, { status: 400 });
    }
    ...
    await prisma.parentMessage.create({
      data: {
        ...
        status: "SENT", sentAt: new Date(), sentByName: "Gurukul Admissions",
      },
    });
    ...
    if (student.parentEmail && process.env.NEXT_PUBLIC_APP_URL) {
      try {
        ...
        console.error("[api/students] admission portal email failed:", mailError);
      }
    }
