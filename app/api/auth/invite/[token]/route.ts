...
async function findValidInvite(token: string) {
  ...
  if (invite.usedAt) return { error: "Invitation already used", status: 410 as const };
  if (invite.expiresAt < new Date()) return { error: "Invitation expired", status: 410 as const };
  return { invite };
}
...
export async function POST(req: Request, { params }: { params: { token: string } }) {
  try {
    ...
    return NextResponse.json({ ok: true, user: { id: user.id, name: user.name, role: user.role } }, { status: 201 });
  } catch (error) {
