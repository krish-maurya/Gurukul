...
export async function GET(_: Request, { params }: { params: { token: string } }) {
  try {
    const student = await prisma.student.findUnique({
      ...
      include: {
        feeAccount: { include: { payments: { orderBy: { paidAt: "desc" }, take: 5 } } },
      },
    });
    if (!student) {
      return NextResponse.json({ error: "This link is not valid. Please ask the school for a new one." }, { status: 404 });
    }
    ...
    const [entries, messages, timetable] = await Promise.all([
      ...
      prisma.parentMessage.findMany({
        where: { studentId: student.id, status: { in: ["SENT", "ACKNOWLEDGED"] } },
        orderBy: { sentAt: "desc" },
        take: 50,
        select: { id: true, type: true, title: true, body: true, status: true, sentAt: true, acknowledgedAt: true, sentByName: true },
      }),
      prisma.timetableSlot.findMany({
        where: { grade: student.grade },
        include: { subject: { select: { name: true } }, teacher: { select: { name: true } }, room: { select: { roomNumber: true } } },
    ...
    return NextResponse.json({
      ...
      attendance: {
        ...
        recent: entries.slice(0, 14).map((e) => ({ date: e.attendanceRecord.date, status: e.status })),
      },
      fees: student.feeAccount
        ? {
            ...
            remaining: Math.max(0, student.feeAccount.amountDue - student.feeAccount.amountPaid),
            dueDate: student.feeAccount.dueDate,
            status: student.feeAccount.status,
            payments: student.feeAccount.payments.map((p) => ({ amount: p.amount, paidAt: p.paidAt, method: p.method, receiptNo: p.receiptNo })),
          }
        : null,
      messages,
      timetable: timetable.map((t) => ({
        day: t.day, period: t.period, subject: t.subject.name, teacher: t.teacher.name, room: t.room.roomNumber,
      })),
    });
  } catch (error) {
...
export async function POST(req: Request, { params }: { params: { token: string } }) {
  try {
    const student = await prisma.student.findUnique({ where: { portalToken: params.token }, select: { id: true } });
    if (!student) return NextResponse.json({ error: "Invalid link" }, { status: 404 });
