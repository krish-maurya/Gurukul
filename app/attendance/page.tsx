...
function AttendanceContent() {
  ...
  return (
    <div className="space-y-5 pb-28 animate-fade-in">
      {/* Page Header */}
      <div className="flex flex-col gap-3 border-b pb-4 sm:flex-row sm:items-center sm:justify-between" style={{ borderColor: "var(--line)" }}>
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-base font-semibold tracking-tight text-gurukul-ink" style={{ fontFamily: "var(--font-syne)" }}>Daily Attendance</h1>
            <span className="badge-dark text-[9px]">One per day</span>
          </div>
          <p className="mt-0.5 text-[11px]" style={{ color: "var(--faint)" }}>Tap a roll number to toggle present/absent.</p>
        </div>
        {isSubmitted && (
          <button
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 card p-3">
        <div className="flex flex-wrap items-center gap-3 text-xs">
          ...
          <div className="flex items-center gap-2">
            <Calendar className="h-3.5 w-3.5" style={{ color: "var(--faint)" }} />
            <input
              type="date"
              value={date}
        ...
        <div className="text-[11px]" style={{ color: "var(--faint)" }}>
          <strong className="text-gurukul-ink">{currentUser?.name || "Faculty"}</strong>
        </div>
      </div>

      {/* Submitted notice */}
      {isSubmitted && (
        <div className="flex items-center gap-2 card p-3 text-xs" style={{ color: "var(--muted)" }}>
      ...
      <div className="card p-5">
        <div className="mb-3 flex items-center justify-between border-b pb-2.5" style={{ borderColor: "var(--hover)" }}>
          <div>
            <h3 className="text-[10px] font-medium uppercase tracking-wider" style={{ color: "var(--muted)" }}>{grade} Roll Grid</h3>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="flex min-h-52 items-center justify-center text-xs" style={{ color: "var(--faint)" }}>
          ...
          <div className="grid grid-cols-4 gap-2 sm:grid-cols-8">
            {students.map((student) => {
              ...
              return (
                ...
                >
                  <span className="text-[9px] font-medium" style={{ color: isAbsent ? "rgba(255,255,255,0.7)" : "var(--faint)" }}>
                    ROLL
                  </span>
                  <span className={`font-mono text-lg font-bold leading-tight ${isAbsent ? "text-white" : "text-gurukul-ink"}`}>
                    #{student.rollNumber}
                  </span>
                  <span className={`mt-0.5 text-[8px] font-medium tracking-wider`} style={{ color: isAbsent ? "rgba(255,255,255,0.85)" : "var(--faint)" }}>
                    {student.status}
                  </span>
                </button>
              );
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 flex items-center justify-between gap-4 border-t bg-white p-4 shadow-subtle transition-[left] duration-200 md:left-[var(--sidebar-width)] md:px-8" style={{ borderColor: "var(--line)" }}>
        <div className="flex min-w-0 items-center gap-4 sm:gap-6">
          <div>
            ...
            <p className="mt-0.5 truncate text-xs font-semibold text-gurukul-ink">{grade} &middot; {date}</p>
          </div>
          <div className="hidden h-6 w-px sm:block" style={{ background: "var(--line)" }} />
          <div className="hidden gap-4 text-[11px] sm:flex">
            <span style={{ color: "var(--faint)" }}>Roll: <strong className="text-gurukul-ink">{students.length}</strong></span>
            <span style={{ color: "var(--muted)" }}>Present: <strong className="text-gurukul-ink">{presentCount}</strong></span>
            <span style={{ color: "var(--faint)" }}>Absent: <strong className="text-gurukul-ink">{absentRollNumbers.length}</strong></span>
          </div>
        </div>
        <button
