...
export default async function TeacherDetailsPage({ params }: { params: { id: string } }) {
  ...
  return <div className="max-w-3xl space-y-6">
    <div className="border-b pb-5 flex items-center gap-3" style={{ borderColor: "var(--line)" }}>
      <div className="w-10 h-10 rounded-lg text-white flex items-center justify-center" style={{ background: "var(--accent)" }}>
        <Users className="w-5 h-5" />
      </div>
      <div>
        <h1 className="text-xl font-bold text-gurukul-ink" style={{ fontFamily: "var(--font-syne)" }}>{teacher.name}</h1>
        <p className="text-xs" style={{ color: "var(--faint)" }}>Teacher Information</p>
      </div>
    </div>
